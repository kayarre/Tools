...
reader.SetFileName("/raid/home/ksansom/caseFiles/mri/PAD_proj/case1/fluent3/vtk_out/wall_outfile_node.vtu")
reader.Update()
...
calc2.SetResultArrayType(vtk.VTK_FLOAT)
...
def array_avg(out_array, in_array, sz_array, n_time):
    for i in range(sz_array):
        calc = out_array.GetValue(i) + (in_array.GetValue(i) - out_array.GetValue(i))/n_time
        #print(calc)
        out_array.SetValue(i, calc)

writer = vtk.vtkXMLUnstructuredGridWriter()
#writer.SetFileName(os.path.join(out_dir,'test_outfile.vtu'))
writer.SetFileName("/raid/home/ksansom/caseFiles/mri/PAD_proj/case1/fluent3/vtk_out/calc_test_node2.vtu")
...
for i in range(N):
    ...
    if( i > 0):
        array_avg(grid.GetPointData().GetArray("TAWSS"),
                  calc2.GetOutput().GetPointData().GetArray("WSS"),
                  grid.GetNumberOfPoints(), float(i+1.0))
        array_avg(grid.GetPointData().GetArray("TAWSSG"),
                  calc2.GetOutput().GetPointData().GetArray("WSSG"),
                  grid.GetNumberOfPoints(), float(i+1.0))
        array_avg(grid.GetPointData().GetArray("x_shear_avg"),
                  calc2.GetOutput().GetPointData().GetArray("x_wall_shear"),
                  grid.GetNumberOfPoints(), float(i+1.0))
        array_avg(grid.GetPointData().GetArray("y_shear_avg"),
                  calc2.GetOutput().GetPointData().GetArray("y_wall_shear"),
                  grid.GetNumberOfPoints(), float(i+1.0))
        array_avg(grid.GetPointData().GetArray("z_shear_avg"),
...
calc3.SetResultArrayType(vtk.VTK_FLOAT)
calc3.Update()
...
calc4.SetResultArrayType(vtk.VTK_FLOAT)
calc4.Update()
...
writer2.SetFileName("/raid/home/ksansom/caseFiles/mri/PAD_proj/case1/fluent3/vtk_out/calc_test_node_stats.vtu")
